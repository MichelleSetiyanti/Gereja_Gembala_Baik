...
function updateBaptismCertificate(event) {
    ...
    for (const key in mappings) {
        if (foundBaptisData[key] !== undefined) {
            ...
            else if ((key.includes("tanggal") || key.includes("Tanggal")) && foundBaptisData[key] && foundBaptisData[key] !== "0000-00-00")
                document.getElementById(mappings[key]).textContent = new Date(foundBaptisData[key]).toLocaleDateString("id-ID", { year: "numeric", month: "long", day: "numeric" });
            else
                document.getElementById(mappings[key]).textContent = foundBaptisData[key];
        }

        if (namaBaptis[0] && namaBaptis[1]) {
            document.getElementById("printNamaLengkap").textContent = `${namaBaptis[0]} ${namaBaptis[1]}`;
        }
    }

    $("#signatureDate").text(new Date().toLocaleDateString("id-ID", { year: "numeric", month: "long", day: "numeric" }));
...
function approveSakramenDewasa(event) {
    ...
    fetch("./php/sakramen_dewasa/approve_sakramen_dewasa.php", { method: "POST", body: formData })
        .then((response) => {
            return response.json()
        }).then((data) => {
            $("#nikVerificationPopUp").css("display", "flex");
...
function laporanBaptis(year) {
    fetchData(`./php/laporan/LaporanBaptis.php?year=${year}`, "#baptismTableBody", ["created_at", "id_admin", "nik", "nama_lengkap", "nama_baptis", "nama_ayah", "nama_ibu", "is_exist", "status"], undefined, undefined, undefined, undefined, undefined, (data) => {
...
function baptisRollCall(year) {
    fetchData(`./php/laporan/baptis_roll_call.php?year=${year}`, "#baptismRollCallTable", ["id", "nama_lengkap", "nik", ""], undefined, undefined, undefined, undefined, undefined, (data) => {
        $(".baptismCertificate").attr("id", "")
        $(".print-report").attr("id", "")
        $(".print-roll-call").attr("id", "print-roll-call")

        $("#roll-call-date").text(new Date().toLocaleDateString("id-ID", { year: "numeric", month: "long", day: "numeric", hour: "numeric", minute: "numeric" }));
...
function handleBaptisNikSearch(event) {
    ...
    debounceSearch = setTimeout(() => {
        ...
        if (foundData && foundDataType === "dewasa" && foundData.status !== "3") return
