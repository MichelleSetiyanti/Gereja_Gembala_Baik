...
const baptisType =
  JSON.parse(localStorage.getItem("user"))["status"] === "4"
    ? "bayi"
    : "dewasa";
...
function fetchData(
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.success) {
        ...
        if (tableName === "data_biduk") {
          ...
          $("#baptisCreateKatekumenId")
            .html(
              response.data.map((item) => {
                if (item.status === "3")
                  return `<option value="${item.id}">${item.nama_diri}</option>`;
              })
            )
            .filter((item) => !item);
        ...
        if (response?.data?.length === 0) {
          $(tableBodyId).html(
            `<tr><td colspan='${columns.length + 1}'>Data kosong</td></tr>`
          );
        } else {
          response.data.forEach(function (item) {
            ...
            if (item) {
              columns.forEach((column) => {
                if (
                  (column === "file_kk_ktp" ||
                    column === "file_surat_pengantar" ||
                    column === "kartu_katekumen") &&
                  !!item[column]
                ) {
                  console.log(item[column]);
                  rows += `<td><a href='${item[column].replaceAll(
                    "../",
                    ""
                  ...
                  rows += `<td>${
                    item[column]
                      ? new Date(item[column]).toLocaleDateString("id-ID")
                      : ""
                  }</td>`;
                } else {
            ...
            if (
              location.pathname !== "/" &&
              JSON.parse(localStorage.getItem("user"))["status"] === "6"
            ) {
              if (editCallback || deleteCallback || printFunction) {
                rows += '<td style="display:flex;gap:5px;">';
                if (editCallback) {
                  rows += `<button class='edit-btn-${url
                    .split(".")[1]
                    .split("/")
                    .pop()}' data-id='${item.id}' ${generateDataAttributes(
                    item
                  )}>Edit</button>`;
                }
                if (deleteCallback) {
                  rows += `<button data-table='${tableName}' class='delete-btn-${url
                    .split(".")[1]
                    .split("/")
                    .pop()}'
                ...
                if (printFunction && item.status === "3") {
                  if (
                    url.includes("tabelSuratBaptis.php") &&
                    JSON.parse(localStorage.getItem("user"))["status"] === "6"
                  ) {
                    rows += `<button data-id='${
                      item.id
                    }' onclick='updateBaptismCertificate(event)' class='print-btn-${url
                      .split(".")[1]
                      .split("/")
                      .pop()}'>Print</button>`;
                  }
                ...
                if (
                  ...
                  item.status === "1" &&
                  JSON.parse(localStorage.getItem("user"))["status"] === "6" &&
                  (tableName === "surat_baptis" ||
                    tableName === "sakramen_dewasa")
                ) {
                  rows += `<button data-id='${
                    item.id
                  }' data-status='3' class='approve-btn-${url
                    .split(".")[1]
                    .split("/")
                    .pop()}' >Setujui</button>`;
                  rows += `<button data-id='${
                    item.id
                  }' data-status='2' class='reject-btn-${url
                    .split(".")[1]
                    .split("/")
                    .pop()}' >Tolak</button>`;
                } else if (
                  ...
                  rows += `<button data-id='${
                    ...
                    .split(".")[1]
                    .split("/")
                    .pop()}' onclick='approveSakramenDewasa(event)'>Setujui</button>`;
                  rows += `<button data-id='${
                    item.id
                  }' data-status='2' class='reject-btn-${url
                    .split(".")[1]
                    .split("/")
                    .pop()}' onclick='approveSakramenDewasa(event)'>Tolak</button>`;
                }
              }
              rows += "</td>";
            }
          ...
          if (approveFunction) {
            console.log(`.approve-btn-${url.split(".")[1].split("/").pop()}`);
            $(`.approve-btn-${url.split(".")[1].split("/").pop()}`).click(
              approveFunction
            );
            $(`.reject-btn-${url.split(".")[1].split("/").pop()}`).click(
              approveFunction
            );
          }
          if (editCallback)
            $(`.edit-btn-${url.split(".")[1].split("/").pop()}`).click(
              editCallback
            );
          // if (printFunction) $(`.print-btn-${url.split(".")[0]}`).click(() => updateBaptismCertificate(itemId));
          if (deleteCallback)
            $(`.delete-btn-${url.split(".")[1].split("/").pop()}`).click(
              deleteCallback
            );
        }
      } else {
        $(tableBodyId).html(
          `<tr><td colspan='${columns.length + 1}'>Data gagal dimuat</td></tr>`
        );
      }
    ...
    error: function () {
      $(tableBodyId).html(
        `<tr><td colspan='${columns.length + 1}'>Terjadi kesalahan</td></tr>`
      );
    },
  });
}
...
$(document).ready(function () {
  ...
  fetchData(
    isAdmin
      ? `./php/sakramen_dewasa/tabelDewasa.php?admin_id=${user["id_admin"]}`
      : "./php/sakramen_dewasa/tabelDewasa.php",
    "#table-body-dewasa",
    [
      "id",
      "id_admin",
  ...
  fetchData(
    isAdmin
      ? `./php/pembayaran/tabelPembayaranSuratBaptis.php?admin_id=${user["id_admin"]}`
      : "./php/pembayaran/tabelPembayaranSuratBaptis.php",
    "#table-body-pembayaran",
    ["id", "nik", "nama_lengkap", "tanggal_baptis"],
    editDataPembayaran,
    deleteTable,
    undefined,
    "pembayaran_surat_baptis",
    undefined,
    undefined,
    undefined
  );
  ...
  fetchData(
    isAdmin
      ? `./php/pastor/tabelPastorParoki.php?admin_id=${user["id_admin"]}`
      : "./php/pastor/tabelPastorParoki.php",
    "#table-body-pastor",
    ["id", "nama_pastor"],
    editPastorParoki,
    deleteTable,
  ...
  fetchData(
    isAdmin
      ? `./php/katekumen_anak/tabelKateAnak.php?admin_id=${user["id_admin"]}`
      : "./php/katekumen_anak/tabelKateAnak.php",
    "#table-body-kateanak",
    [
      "id_kateanak",
      "id_admin",
  ...
  fetchData(
    isAdmin
      ? `./php/katekumen_dewasa/tabelKateDewasa.php?admin_id=${user["id_admin"]}`
      : "./php/katekumen_dewasa/tabelKateDewasa.php",
  ...
  fetchData(
    isAdmin
      ? `./php/surat_baptis/tabelSuratBaptis.php?admin_id=${user["id_admin"]}`
      : "./php/surat_baptis/tabelSuratBaptis.php",
    "#table-body-surat",
    [
      "id",
      "id_admin",
  ...
  fetchData(
    isAdmin
      ? `./php/sakramen_bayi/tabelBayi.php?admin_id=${user["id_admin"]}`
      : "./php/sakramen_bayi/tabelBayi.php",
    "#table-body-bayi",
    [
      "id",
      "id_admin",
  ...
  const modals = {
    ...
    databasePembayaranSuratBaptis: getElement(
      "formDatabasePembayaranSuratBaptis"
    ),
  ...
  handleFormSubmission(
    "#formModal5 .form-container",
    "./php/sakramen_bayi/sakramen_bayi.php",
    "#formModal5 #messageBox"
  );
  handleFormSubmission(
    "#formModal1 .form-container",
    "./php/sakramen_dewasa/sakramen_dewasa.php",
    "#formModal1 #messageBox"
  );
  handleFormSubmission(
    "#formModal3 .form-container",
    "./php/katekumen_anak/create_katekumen_anak.php",
    "#formModal3 #messageBox"
  );
  handleFormSubmission(
    "#formModal4 .form-container",
    "./php/katekumen_dewasa/create_katekumen_dewasa.php",
    "#formModal4 #messageBox"
  );
  handleFormSubmission(
    "#formModal2 .form-container",
    "./php/surat_baptis/surat_baptis.php",
    "#formModal2 #messageBox"
  );
  handleFormSubmission(
    "#formModalBiduk .form-container",
    "./php/biduk/database_biduk.php",
    "#formModalBiduk #messageBox"
  );
  handleFormSubmission(
    "#formPembayaranBaptis .form-container",
    "./php/pembayaran/pembayaran_surat_baptis.php",
    "#formPembayaranBaptis #messageBox"
  );
  handleFormSubmission(
    "#editDatabaseBayi .form-container",
    "./php/sakramen_bayi/UpdateTabelBayi.php",
    "#editDatabaseBayi #messageBox"
  );
  handleFormSubmission(
    "#editDatabaseDewasa .form-container",
    "./php/sakramen_dewasa/UpdateTabelDewasa.php",
    "#editDatabaseDewasa #messageBox"
  );
  handleFormSubmission(
    "#editDatabaseKatekumenDewasa .form-container",
    "./php/katekumen_dewasa/UpdateTabelKatekumenDewasa.php",
    "#editDatabaseKatekumenDewasa #messageBox"
  );
  handleFormSubmission(
    "#editDatabaseKatekumenAnak .form-container",
    "./php/katekumen_anak/UpdateTabelKatekumenAnak.php",
    "#editDatabaseKatekumenAnak #messageBox"
  );
  handleFormSubmission(
    "#editDatabaseBaptis .form-container",
    "./php/surat_baptis/UpdateTabelBaptis.php",
    "#editDatabaseBaptis #messageBox"
  );
  handleFormSubmission(
    "#editDatabaseBiduk .form-container",
    "./php/biduk/UpdateTabelBiduk.php",
    "#editDatabaseBiduk #messageBox"
  );
  handleFormSubmission(
    "#formEditPembayaranBaptis .form-container",
    "./php/pembayaran/UpdatePembayaranSuratBaptis.php",
    "#formEditPembayaranBaptis #messageBox"
  );
  handleFormSubmission(
    "#formEditPastorParoki .form-container",
    "./php/pastor/UpdatePastorParoki.php",
    "#formEditPastorParoki #messageBox"
  );
  handleFormSubmission(
    "#formPastorParoki .form-container",
    "./php/pastor/pastor_paroki.php",
    "#formPastorParoki #messageBox"
  );
  ...
  function refetchUserData() {
    $.ajax({
      url:
        "./php/user/getUserData.php?userId=" +
        JSON.parse(localStorage.getItem("user"))["id_admin"],
      type: "GET",
      dataType: "json",
      success: function (response) {
